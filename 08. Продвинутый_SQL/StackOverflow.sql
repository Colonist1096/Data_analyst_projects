...
FROM (SELECT v.user_id,
      ...
      ORDER BY v_cnt DESC
      LIMIT 10) AS vo
ORDER BY vo.v_cnt DESC, vo.user_id DESC;
...
SELECT groupes.id, 
       ...
       groupes.group
FROM groupes
WHERE groupes.views = groupes.max
ORDER BY groupes.views DESC, groupes.id;
...
SELECT *,
       SUM(d.cnt_reg) OVER (ORDER BY d.days) as cnt_users
...
WITH d AS (SELECT DISTINCT user_id,
                   MIN(creation_date) OVER (PARTITION BY user_id) AS min_dt      
            FROM stackoverflow.posts)

SELECT d.user_id,
       (d.min_dt - u.creation_date) AS diff
...
WITH ad AS (SELECT user_id,
                   COUNT(DISTINCT creation_date::date)  AS cr_date
            FROM stackoverflow.posts
            WHERE creation_date::date BETWEEN '2008-12-01' AND '2008-12-07' 
            GROUP BY user_id)

SELECT ROUND(AVG(ad.cr_date))
FROM ad;
...
dt AS (SELECT p.user_id,
       ...
       FROM stackoverflow.posts AS p
       JOIN leader ON leader.user_id = p.user_id
